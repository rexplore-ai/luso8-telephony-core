...
Complete step-by-step guide to deploy Asterisk on Google Cloud and wire it into the Luso8 Cloud platform with full CI/CD, monitoring, and call recording.
...
GitHub (main branch push)
        ...
        │   6. Health check (ARI + SIP)
        │
        ▼ GCE VM — africa-south1-a
        │   luso8-asterisk-pbx (e2-standard-2)
        │   Static IP → pbx.luso8.rexplore.ai (Cloudflare A record, proxy OFF)
        │   Docker: --network host (required for SIP/RTP)
        │
        ├─ Port 5060 UDP/TCP  → SIP Trunk (local carrier)
        ├─ Port 10000-10100 UDP → RTP media audio
        └─ Port 8088 TCP (internal only) → ARI → Voxtra (Cloud Run)

GCS Bucket: luso8-call-recordings-{project_number}
...
Add these in: **GitHub repo → Settings → Secrets and variables → Actions → Secrets**

| Secret Name | Value | How to Get |
|---|---|---|
| `GCP_SA_KEY` | JSON key content | Generated by `scripts/gcp-setup.sh` → `/tmp/luso8-github-sa-key.json` |
| `GCE_SSH_KEY` | SSH private key (ed25519) | Printed during `scripts/gcp-setup.sh` |

These are the **only** two GitHub Secrets needed. All other configuration (ARI password, SIP trunk credentials, etc.) lives in GCP Secret Manager and is pulled to the VM at deploy time.
...
# Run infrastructure setup (takes ~5 minutes)
./scripts/gcp-setup.sh
```

The script will output:
1. A JSON service account key → save as GitHub Secret `GCP_SA_KEY`
2. An SSH private key → save as GitHub Secret `GCE_SSH_KEY`
3. The GCE static IP → use for Cloudflare DNS

**Immediately after the script runs:**

```bash
# DELETE the SA key from local disk (it's now in GitHub Secrets)
rm /tmp/luso8-github-sa-key.json
```
...
Go to: `https://github.com/rexplore-ai/luso8-telephony-core/settings/secrets/actions`

Add:
- `GCP_SA_KEY` → paste the JSON content from `/tmp/luso8-github-sa-key.json`
- `GCE_SSH_KEY` → paste the ed25519 private key printed by `gcp-setup.sh`
